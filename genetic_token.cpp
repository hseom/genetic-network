...
int main(int argc, char* argv[]) {
	...
	*/
		...
		for(int h = 0 ; h < nodes ; h++) {
			cout << parent_fit[h] << "\t" << parent_priority[h] << endl;
		}
		cout << "========================================" << endl;
			...
			for (item_it = items.begin(); item_it != items.end(); item_it++) {
					...
					diff = -1;
						...
						Ran1Random ran_no3(diff);
					        ...
					        auto_ptr<SimpleMessage> query_m (new SimpleMessage(query_origin->getTTL(), ran_no2));
